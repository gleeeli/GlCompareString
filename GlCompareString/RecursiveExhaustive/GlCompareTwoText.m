...
//        @try {
            //NSLog(@"开始 current thred:%@",[NSThread currentThread]);
            self.runRecursion = 1;
...
//        } @finally {
//            NSLog(@"@finally:%@",[NSThread currentThread]);
//        }
    ...
    if (self.runRecursion == self.completeRecursion) {
        ...
        if (self.completeBlock) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                weakSelf.completeBlock(weakSelf.bestModel);
            });
        }
    }
}
...
//    @synchronized (self) {
        ...
        if ([nowEvaluateChar isEqualToString:nowRightChar]) {
             ...
             */
            BOOL isHaveErrorCase = nowContinueMatchCount < self.establishMatchLenght || self.establishMatchLenght <= 0;
            if (isHaveErrorCase) {//增加一种结果
                self.runRecursion ++;
            }
